Sovereign.grid.AsianArtworkJudges = function(config) {
    config = config || {};
    ...
    this.currentGalleryId = Ext.getCmp('sovereign-panel-asia').config.currentJudgesGallery;
    Ext.applyIf(config,{
        id: 'sovereign-grid-asianartworkjudges'
        ,url: Sovereign.config.connectorUrl
        ,baseParams: {
            action: 'mgr/asia/artworks/getListArtworksJudges'
        ...
        ,tbar:[{
            ...
            xtype: 'textfield'
            ,id: 'asianartworksjudges-search-filter'
            ...
            xtype: 'button'
            ,id: 'modx-filter-clear-asianartworksjudges'
            ,iconCls:'icon-reload'
            ,text: _('filter_clear')
            ,listeners: {
    ...
    Sovereign.grid.AsianArtworkJudges.superclass.constructor.call(this,config)
};
Ext.extend(Sovereign.grid.AsianArtworkJudges,MODx.grid.Grid,{
    search: function(tf,nv,ov) {
        ...
        Ext.getCmp('asianartworksjudges-search-filter').reset();
        this.getBottomToolbar().changePage(1);
        this.refresh();
    ...
    },showAsianJudges: function(e) {
        var win = MODx.load({
            galleryId: this.config.galleryId
            ,xtype: 'sovereign-window-asian-showjudges'
            ,listeners: {
        ...
        this.displayArtworkWindow = new Sovereign.window.DisplayAsianArtworkJudges;
        ...
        Ext.getCmp('sovereign-panel-asia').backToJudgesGrid();
    }
});
Ext.reg('sovereign-grid-asianartworkjudges',Sovereign.grid.AsianArtworkJudges);


Sovereign.window.DisplayAsianArtworkJudges = function(config) {
    config = config || {};
    var check = Ext.getCmp('sovereign-window-asianartworksubmissions-display');
    if (check) {
        check.destroy();
    }
    this.currentFileName = Ext.getCmp('sovereign-grid-asianartworkjudges').config.currentFileName;
    this.galleryId = Ext.getCmp('sovereign-grid-asianartworkjudges').config.galleryId;
    ...
    Sovereign.window.DisplayAsianArtworkJudges.superclass.constructor.call(this,config);
};
Ext.extend(Sovereign.window.DisplayAsianArtworkJudges,MODx.Window);
Ext.reg('sovereign-window-asianartworkjudges-display',Sovereign.window.DisplayAsianArtworkJudges);
